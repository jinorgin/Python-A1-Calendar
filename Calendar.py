...
def command_add(date, start_time, end_time, title, calendar):
    ...
    if start_time > end_time:
        return False
    #check if they entered the proper time
    elif start_time > 24 and end_time > 24:
        return False
    elif start_time < 0 and end_time < 0:
        return False
    else:
        if date in calendar:
                calendar[date] += [cal] #add title into calendar
                calendar[date] = sorted(calendar[date], key=lambda k: k['start'])
        else:
            calendar[date] = [cal]  #create date
    return True
...
def command_show(calendar):
    ...
    for date in d:
        ...
        display += date + ' : '
        display += '\n'
        #print the detail (start, end, title)
        for detail in calendar[date]:
            display += '    '
            display += "start : "+str(detail['start']) +":00"+",\n"+"    end : "+str(detail['end'])+":00"+",\n"+"    title : "+ detail['title'] + "\n"
            display += '\n'
...
def save_calendar(calendar):
    ...
    count = ""
    file = "calendar.txt"
    cf = open(file, 'w')  # cf: Calendar file
    for date in sorted(calendar.keys(), reverse=True):
        count = count + date + ":"
        for detail in calendar[date]:
            count = count + str(detail['start']) + "-" + str(detail['end']) + " "+ detail['title'] + "\t"
        count = count[:-1]
        count += '\n'
...
def is_calendar_date(date):
    ...
    if len(date) == 10:
        if date[4] == '-' and date[7] == '-':
            ...
            m=date[5:7]
            d=date[8:]
            if yr.isdigit() and m.isdigit() and d.isdigit():
                year = int(yr)
                month = int(m)
                day = int(d)

                if year >= 1800 and year <=9999 and month > 0 and month <= 12 and day >0 and day <= 31:
                    return True
                else:
                    return False
            else:
...
def is_natural_number(str):
    ...
    # YOUR CODE GOES HERE
    naturalNumber = "1234567890"
    if len(str) > 0:
        for i in str:
            if i not in naturalNumber:
                return False
    else:
        return False
...
def parse_command(line):
    ...
    # the final argument.
    # YOUR CODE GOES HERE
    c_input = line.split(" ", 4)
    if len(c_input) > 0:
        if is_command(c_input[0]) is False:
            return ['help']
        if c_input[0] == 'add':
            if len(c_input) < 4:
                return ['error', 'add DATE START_TIME END_TIME DETAILS']
            if is_calendar_date(c_input[1]) is False:
                return ['error', 'not a valid calendar date']
            return ['add', c_input[1], int(c_input[2]), int(c_input[3]), c_input[4]]
        if c_input[0] == 'show':
            if len(c_input) == 1:
                return ['show']
            else:
        ...
        if c_input[0] == 'delete':
            ...
            if is_calendar_date(c_input[1]) is False:
                return ['error', 'not a valid calendar date']
            if is_natural_number(c_input[2]) is False:
                return ['error', 'not a valid event start time']
            return ['delete', c_input[1], int(c_input[2])]
        if c_input[0] == 'quit':
            return ['quit']
        if c_input[0] == 'help':
            return ['help']
