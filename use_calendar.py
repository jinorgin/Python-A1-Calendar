...
def user_interface():
    ...
    '''
    # Your code goes here
    __cal = calendar.load_calendar()

    print("calendar is loaded")
    cmd = input("Choose either add, delete, quit, help, or show: ")
    cmd = calendar.parse_command(cmd)

    while cmd[0] != 'quit':
        if cmd[0] == 'help':
            print(calendar.command_help())
        elif cmd[0] == 'show':
            print(calendar.command_show(__cal))
        elif cmd[0] == 'add':
            calendar.command_add(cmd[1],int(cmd[2]), int(cmd[3]), cmd[4], __cal)
            print("Successfully added!")
        elif cmd[0] == 'delete':
            calendar.command_delete(cmd[1], int(cmd[2]), __cal)
            print("Successfully deleted!")
        cmd = input()
        cmd = calendar.parse_command(cmd)
    calendar.save_calendar(__cal)
